...
/**
 * Функция должна создать элемент с тегом A, 
 установить значение для атрибута href и вернуть получившийся элемент
 *
...
function findAllPSiblings(where) {
   ...
   let arr = [];
    ...
    for(let i = 0;i<childNodes.length-1; i++) {
        ...
        if (childNodes[i].nextSibling.tagName === 'P') {
 ...
 * и вернуть массив, состоящий из текстового содержимого перебираемых 
 элементов
...
function collectDOMStat(root) {
    ...
    function findTagsClass(element) {
        let children = element.children;
        for(let i = 0; i<children.length; i++) {
          if (res.classes.hasOwnProperty('"' + children[i].getAttribute('class') + '"') ) {
            res.classes['"' + children[i].getAttribute('class') + '"']++;
          } else  if (children[i].getAttribute('class')){
            res.classes['"' + children[i].getAttribute('class') + '"'] = 1;
          }
          if (res.tags.hasOwnProperty(children[i].tagName)) {
            res.tags[children[i].tagName]++
          } else {
            res.tags[children[i].tagName] = 1;
          }
          findTagsClass(children[i]);
        }
    }
 ...
 * @param {function(info: {type: string, nodes: Array<Element>})} fn - функция, которую 
 необходимо вызвать
 *
 * @example
